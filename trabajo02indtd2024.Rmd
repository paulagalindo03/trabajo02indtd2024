...
```
<br>
**CONCLUSIÓN:** en el núcleo aproximado se encuentran las ubicaciones C y D, es decir, las que no son dominadas por otras. Por este motivo, estas dos alternativas son las mejores para abrir el futuro restaurante. 

Volvemos a realizar el proceso reduciendo el número de las alternativas por las que han resultado en el núcleo aproximado.
```{r}
solElectre_2 <- multicriterio.metodoELECTRE_I(matrizDecision,
                                      pesos.criterios = tab.fpref[,3]/sum(tab.fpref[,3]),
                                      nivel.concordancia.minimo.alpha = 0.7,
                                      no.se.compensan = c(Inf, Inf, Inf, Inf, Inf),
                                      que.alternativas = c(3,4))
solElectre_2
qgraph::qgraph(solElectre_2$relacion.dominante)
solElectre_2$nucleo_aprox
```
<br>
**CONCLUSIÓN:** en esta iteración hemos tomado solo las dos alternativas, C y D, pero nos vuelve a salir el mismo núcleo aproximado. Probamos ahora reduciendo el valor de alpha (nivel de concordancia mínimo) a 0.6.

```{r}
solElectre_3 <- multicriterio.metodoELECTRE_I(matrizDecision,
                                      pesos.criterios = tab.fpref[,3]/sum(tab.fpref[,3]),
                                      nivel.concordancia.minimo.alpha = 0.6,
                                      no.se.compensan = c(Inf, Inf, Inf, Inf, Inf),
                                      que.alternativas = c(3,4))
solElectre_3
qgraph::qgraph(solElectre_3$relacion.dominante)
solElectre_3$nucleo_aprox
```
<br>
**CONCLUSIÓN:** hemos conseguido reducir el núcleo aproximado a una sola alternativa que es la ubicación D. Por tanto podemos decir que la mejor decisión que puede tomar la cadena para abrir el nuevo restaurante es la ubicación D.
