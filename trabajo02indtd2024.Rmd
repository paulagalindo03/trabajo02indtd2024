...
**CONCLUSIÓN:** se observa que la ubicación A sigue siendo es la opción más fuerte, ya que es la que domina a todas las demás y además gráficamente se ve claramente cómo domina en comparación con las demás ubicaciones. La ubicación C es la alternativa más débil ya que no supera a ninguna otra. 
...
```
**CONCLUSIÓN:** en cuanto a los criterios podemos decir que el más importante para la cadena de restaurantes es el costo de alquiler con un 30.8%, seguido del tráfico (23.1%) y siendo la competencia el criterio con menor importancia en la decisión final (12.3%).

Si nos centramos en cada uno de los criterios por separados, observamos un desglose porcentual de preferencia para las alternativas. Por ejemplo, para el criterio "CostoAlquiler", la ubicación E es la que mayor peso tiene con un porcentaje de 25.5% mientras que la ubicación con menor peso es la C (14.6%). Para el criterio "Tráfico", la mejor alternativa es la ubicación A (25.7%) y la peor vuelve a ser la ubicación C.

La inconsistencia en todas las comparaciones es de 0.0%, lo que indica que las evaluaciones realizadas son perfectamente consistentes según los principios del AHP.  
...
```
**CONCLUSIÓN:** podemos concluir diciendo que la ubicación A, una zona céntrica de la ciudad, es la mejor alternativa combinando los cinco criterios evaluados para la cadena de restaurantes, con una prioridad global de 23.4%. Esto se debe principalmente a su buen desempeño en los criterios de tráfico (25,7%, la mejor puntuación) y competencia (25.0%). La segunda mejor opción es la ubicación E, que presenta la segunda prioridad global más alta de 21.9%, destacándose en el criterio de accesibilidad con un 26.5%. <br>
Se puede decir que la peor de las alternativas barajadas por la cadena es la ubicación C con el menor peso global de 14.6%, debido a su bajo rendimiento en la mayoría de los criterios evaluados.
...
Tras haber estudiado la situación por tres métodos distintos (Promethee, Electre y AHP), podemos ver que todas las conclusiones coinciden en que la mejor alternativa es la ubicación A, luego la decisión final de la cadena será abrir el nuevo restaurante en una zona céntrica de la ciudad de Sevilla.

Podría haber pasado que algún método resultara una ubicación distinta. Esto se debe a que según el método que usemos para resolver un mismo problema de decisión multicriterio, con las mismas alternativas y criterios, los resultados pueden variar debido a la funcionalidad de cada uno de ellos.
